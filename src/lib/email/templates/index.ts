...
export async function render(
  ...
  try {
    ...
    // Fix: Pass the locale to getTranslations function
    const emailTranslations = await getTranslations({
      locale,
      namespace: "Email",
    });

    // Load the template for the user's locale, with fallback to English
    const htmlTemplate = await getCompiledTemplate(
      `${template}/${locale}.html`,
      `${template}/en.html`
    );
    const textTemplate = await getCompiledTemplate(
      `${template}/${locale}.txt`,
      `${template}/en.txt`
    );
    ...
    throw new Error(
      `Failed to render email template: ${(error as Error).message}`
    );
  }
}
...
async function getCompiledTemplate(
  ...
  try {
    ...
    if (!fallbackPath) {
      throw new Error(
        `Failed to load email template at ${templatePath} and no fallback provided`
      );
    }

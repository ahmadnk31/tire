...
export function SearchComponent({
    ...
    const search = async () => {
      ...
      try {
        const response = await fetch(
          `/api/search?query=${encodeURIComponent(debouncedSearchTerm)}&limit=5`
        );
  ...
  // Handle clicks outside to close dropdown - completely rewritten for reliable behavior
  useEffect(() => {
    // Skip if the dropdown isn't open
    if (!isOpen) return;

    // Define the click handler
    function handleClickOutside(event: MouseEvent) {
      // Check if the click was outside the search component
      if (
        commandRef.current &&
    ...
    // The delay ensures the click that opened the search doesn't also close it
  ...
  // Handle product selection - improved to ensure UI closes properly
  const handleSelect = (productId: string) => {
    router.push(`/products/${productId}`);
    // Explicitly close both dropdown and fullscreen mode
    setIsOpen(false);
    setIsFullscreen(false);
    setQuery("");
  };
  ...
  // Handle view all results click - improved to ensure UI closes properly
  ...
  return (
    <div
      className={cn(
        ...
        isFullscreen &&
          "fixed inset-0 z-50 w-full max-w-none h-full bg-background/95 backdrop-blur-sm p-4 flex flex-col"
      )}
      ref={commandRef}
    >
      {isFullscreen && (
      ...
      >
        <div className='relative w-full'>
          ...
          <input
            ...
            onFocus={() => {
              setIsFocused(true);
              // Always open dropdown on focus, regardless of query content
              setIsOpen(true);
            }}
            onBlur={() => {
              setIsFocused(false);
            ...
            onClick={() => {
              // Always expand the search when clicked, regardless of query content
              setIsOpen(true);

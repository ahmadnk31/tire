    ...
    try {
          ...
          shipping: addressData
            ? {
                ...
                address: {
                  ...
                  line2:
                    addressData.address.line2 === null
                      ? undefined
      ...
      if (error) {
        ...
        if (paymentIntent.status === "processing") {
          ...
          onError(
            `Payment status: ${paymentIntent.status}. Please wait a moment.`,
            {
  ...
  const getCustomerFriendlyErrorMessage = (error: any): string => {
    switch (error.code) {
      ...
      case "authentication_required":
        return "This payment requires authentication. Please complete the verification step.";
      case "rate_limit":
              ...
              ? defaultShippingOptions.map((option) => (
                  <div
                    ...
                    className={`flex items-center justify-between p-3 rounded-md cursor-pointer border ${
                      collectedAddress &&
                      collectedAddress.shippingOption === option.id
...
export default function PaymentPage() {
  ...
  const [paymentErrorDetails, setPaymentErrorDetails] =
    useState<PaymentErrorDetails | null>(null);
  ...
  const handleShippingOptionChange = useCallback(
    (option: ShippingOption) => {
      ...
      updateSummary(
        summary.subtotal + newShippingCost + summary.tax - summary.discount
      );
    },
      ...
      if (shippingAddress && items.length > 0) {
        try {
          ...
          const options: ShippingOption[] = rates.map((rate) => ({
            ...
            description: `${
              rate.transitDays
                ? `Delivery in ${rate.transitDays} days`
                : "Standard delivery"
            }`,
          ...
          if (selectedShippingOption) {
            ...
            const matchedOption = options.find(
              (option) =>
                option.id === selectedShippingOption.id ||
          ...
          toast.error(
            "Unable to fetch shipping options. Using default options."
          );
  ...
  const handleSuccessfulPayment = async () => {
    ...
    try {
      ...
      setPaymentError(
        "Payment was processed but we encountered an issue creating your order. Please contact customer support."
      );
      toast.error(
        "Error finalizing your order. Please contact support with your payment confirmation."
      );
    } finally {
      setIsProcessing(false);
    }
  ...
  const handlePayPalPayment = async (data: any) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage =
          errorData.error || "Failed to capture PayPal payment";
      ...
      const errorMessage =
        error.message || "Failed to process PayPal payment. Please try again.";
      ...
      <div className='container max-w-lg py-16 text-center'>
        <div className='flex flex-col items-center mb-8'>
          <div className='relative'>
            <div className='absolute inset-0 animate-ping rounded-full bg-green-200 opacity-50'></div>
        ...
        <p className='mb-8 text-muted-foreground'>
          Your order has been placed and a confirmation email will be sent to
          you shortly. You'll be redirected to the order confirmation page in a
          moment.
        </p>
                ...
                <div className='space-y-6'>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        updateUserData({ notes: e.target.value })
                      }
                    ></textarea>
                  </div>
                </div>

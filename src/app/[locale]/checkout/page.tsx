...
import {
  ...
  getShippingRates,
} from "@/lib/shipping/shipping-client";
...
export default function CheckoutPage() {
  ...
  const handleAddressSelected = (addressData: AddressData): void => {
    ...
    setShippingAddress({
      ...
      addressLine2:
        shippingAddress?.addressLine2 || form.getValues("addressLine2") || "",
  ...
  const validateAndFetchRates = async (
      ...
      setAddressFeedback({
        ...
        messages: [
          (error as Error).message ||
            "Failed to validate address and get shipping rates",
        ],
      });
    } finally {
  ...
  // Format price to display with 2 decimal places and correct currency symbol
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    const selectedOption = shippingOptions.find(
      (option) => option.id === values.shippingOption
    );
